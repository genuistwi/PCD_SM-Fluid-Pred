from models.Custom_CondAttUnet_lowSkip.model import Network
import torch
...
def model_setup(Cfg, data_set):
    ...
    assert data_set.dataset_name in ["JHTDB", "turbulent_radiative_layer_2D"]
    ...
    if data_set.dataset_name == "turbulent_radiative_layer_2D":
        ...
        model.cond_format = lambda cond: cond.reshape(cond.shape[0], -1, * cond.shape[3:])  # (B, Num Frames, F, H, W) -> (B, NF*F, H, W), works with 1 frame also
        ...
        def energyLoss_format(X0, cond, X_hat):
            ...
            X_mean = X_per_field.mean(dim=1)  # Average across fields
    ...
    if data_set.dataset_name == "JHTDB":
        ...
        def energyLoss_format(X0, cond, X_hat):
            ...
            u_p_p = (cond[:, :len_fields, :, :] - X_mean)[:, :2, ...]  # velocity x and y are first in this dataset
            u_p_t = (X0 - X_mean)[:, :2, ...]
            u_p_h = (X_hat - X_mean)[:, :2, ...]
